            ...
            <div class="bg-white shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white">
                    <form method="POST">
                        ...
                        <div class="md:flex md:items-center mb-6">
                            <div class="md:w-1/3">
                                <label class="block md:text-right mb-1 md:mb-0 pr-4" for="start_time">Godzina
                                    rozpoczęcia</label>
                            </div>
                            <div class="md:w-2/3">
                                <input type="hour" name="start_time" id="start_time" @if ($edit)
                                value="{{ $event->start_time }}" {{ $editable }} @endif
                                class="rounded border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 w-full">
                            </div>
                        </div>
                        <div class="md:flex md:items-center mb-6">
                        ...
                        <div class="md:flex md:items-center mb-6">
                            <div class="md:w-1/3">
                                <label class="block md:text-right mb-1 md:mb-0 pr-4" for="end_time">Godzina
                                    zakończenia</label>
                            </div>
                            <div class="md:w-2/3">
                                <input type="hour" name="end_time" id="end_time" @if ($edit)
                                value="{{ $event->end_time }}" {{ $editable }} @endif
                                class="rounded border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 w-full">
                            </div>
                        </div>
                        <div class="md:flex md:items-center mb-6">
                        ...
                        <div class="md:flex md:items-center mb-6">
                            <div class="md:w-1/3">
                                <label class="block md:text-right mb-1 md:mb-0 pr-4"
                                    for="invites">{{ __('forms.invites') }}</label>
                                <div class="block uppercase font-black text-gray-600 md:text-right text-xs pr-4 pb-2">
                                    {{ __('forms.invites_email') }}</div>
                            </div>
                            <div class="md:w-2/3">
                        ...
                        @if ($errors->any())
                            <div class="flex-initial flex-col bg-red-100 border-l-8 border-red-600 py-1 my-3">
                                @foreach ($errors->all() as $error)
                                    <p class="text-md font-bold text-red-600 text-sm p-1.5 pl-6">{{ $error }}
                                    </p>
                                @endforeach
                            </div>
                        @endif
                        <div class="flex justify-center gap-1.5">
                            ...
                            @if ($edit)
                                <a
                                    ...
                                    href="/events/edit/{{ $event->event }}/delete">{{ __('app.delete') }}</a>
                            @endif
                        </div>
                    </form>
                </div>
